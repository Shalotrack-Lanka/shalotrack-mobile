...
public class VehicleTrailRenderer {
    ...
    // Jitter filter (physics sanity check, not true road-snapping): a fix
    // is rejected if the implied speed since the last accepted fix exceeds
    ...
    public VehicleTrailRenderer(Context context, GoogleMap map, ShaloTrackApi api) {
        this.map = map;
        this.api = api;
        // Requires res/drawable/ic_car_marker.xml.
